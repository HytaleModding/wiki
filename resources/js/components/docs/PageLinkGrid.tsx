...
export default function PageLinkGrid({
  ...
  return (
    <div
      className={`grid gap-x-8 gap-y-3 ${columns === 2 ? 'md:grid-cols-2' : ''}`}
    >
      {pages.map((page) => (
        <a
          key={page.id}
        ...
        >
          <div className="flex items-start justify-between gap-3">
            <h4 className="font-medium break-words text-foreground group-hover:text-primary">
              {page.title}
            </h4>
            <ArrowUpRight className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5 group-hover:text-primary" />
          </div>
